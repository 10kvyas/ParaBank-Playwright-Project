...
=====================================================================================\n`);
    ...
    function _parseFeatures(suite) {
        suite.features.forEach((feature) => {
            ...
            // Detect execution platform (local vs cloud)
            {
                ...
                else if (platformStr.includes('testmu') ||
        ...
        scenario.steps.forEach((step) => {
            if (step.embeddings !== undefined) {
                ...
                embeddings.forEach((embedding, embeddingIndex) => {
                    ...
                    else if (['image/png', 'image/avif', 'image/webp', 'image/jpeg'].includes(embedding.mime_type ?? '') ||
    ...
    async function _createFeaturesOverviewIndexPage(suite) {
        ...
        const report = {
            ...
            release: options.customData?.data?.find((item) => item.label === 'Release')
                ?.value,
            cycle: options.customData?.data?.find((item) => item.label === 'Cycle')?.value,
    ...
    async function _createFeatureIndexPages(suite) {
        for (const feature of suite.features) {
            ...
            const report = {
                ...
                release: options.customData?.data?.find((item) => item.label === 'Release')
                    ?.value,
                cycle: options.customData?.data?.find((item) => item.label === 'Cycle')
                    ?.value,
